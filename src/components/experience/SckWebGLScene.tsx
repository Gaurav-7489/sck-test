...
function Aircraft({ p }: { p: MutableRefObject<Progress> }) {
  ...
  return <group ref={g}>
    <group rotation={[0,0,Math.PI/2]}>
      ...
      <mesh position={[2.75,.06,.9]} material={m.dark}><boxGeometry args={[1.6,.08,.9]}/></mesh>
      {[-1,1].map(side=><group key={side} position={[1.25,-.28,side*1.42]}><mesh rotation={[Math.PI/2,0,0]} material={m.black}><cylinderGeometry args={[.5,.62,1.7,32]}/></mesh><mesh position={[0,0,side*.87]} rotation={[Math.PI/2,0,0]} material={m.chrome}><torusGeometry args={[.45,.055,12,40]}/></mesh><pointLight position={[0,0,side*.92]} intensity={5} distance={3} color="#dbe9ff"/></group>)}
...
function Runway() {
  ...
  return <group position={[0,-1.2,-7]}><mesh rotation={[-Math.PI/2,0,0]}><planeGeometry args={[22,100]}/><meshStandardMaterial color="#030303" roughness={.86} metalness={.25}/></mesh><mesh position={[0,.02,-2]} rotation={[-Math.PI/2,0,0]}><planeGeometry args={[.06,88]}/><meshBasicMaterial color="#fff" transparent opacity={.28}/></mesh>{[-8,8].map(x=><group key={x}>{lights.map(i=><mesh key={i} position={[x,.04,35-i*1.8]}><boxGeometry args={[.08,.03,.32]}/><meshBasicMaterial color="#dce9ff" transparent opacity={.2+(i%3)*.06}/></mesh>)}</group>)}</group>
}

function CloudsScene({ p }: { p: MutableRefObject<Progress> }) {
  const g=useRef<THREE.Group>(null)
  ...
  return <group ref={g}><Clouds limit={600} material={THREE.MeshStandardMaterial}><Cloud position={[-10,5,-10]} seed={11} segments={45} volume={7} color="#fff" opacity={.72} fade={7} growth={4}/><Cloud position={[-3,7,-15]} seed={21} segments={55} volume={8} color="#fff" opacity={.8} fade={8} growth={5}/><Cloud position={[7,5,-12]} seed={31} segments={50} volume={8} color="#fff" opacity={.74} fade={7} growth={5}/><Cloud position={[0,9,-26]} seed={41} segments={60} volume={10} color="#fff" opacity={.78} fade={10} growth={6}/><Cloud position={[12,3,-20]} seed={51} segments={42} volume={7} color="#fff" opacity={.58} fade={8} growth={4}/><Cloud position={[-14,3,-24]} seed={61} segments={44} volume={8} color="#fff" opacity={.62} fade={8} growth={4}/></Clouds></group>
}

function Camera({ p }: { p: MutableRefObject<Progress> }) { const {camera,pointer}=useThree();const sm=useRef(0);useFrame(()=>{const q=p.current;if(!q.done)return;sm.current=THREE.MathUtils.lerp(sm.current,q.scroll,.035);const s=sm.current,orbit=range(.02,.48,s),dep=range(.45,.78,s),fly=range(.76,1,s),a=-.8+orbit*Math.PI*2.2,r=THREE.MathUtils.lerp(15,9,orbit);const pos=new THREE.Vector3(Math.cos(a)*r,3.4+Math.sin(a*.7)*2,-7+Math.sin(a)*r);pos.x=THREE.MathUtils.lerp(pos.x,8,dep);pos.y=THREE.MathUtils.lerp(pos.y,3,dep);pos.z=THREE.MathUtils.lerp(pos.z,4,dep);pos.x=THREE.MathUtils.lerp(pos.x,-6,fly);pos.y=THREE.MathUtils.lerp(pos.y,11,fly);pos.z=THREE.MathUtils.lerp(pos.z,20,fly);pos.x+=pointer.x*.45;pos.y+=pointer.y*.2;camera.position.lerp(pos,.035);camera.lookAt(new THREE.Vector3(0,.4-fly*.2,-7+fly*-14))});return null }

function Scene(){const p=useRef<Progress>({intro:0,scroll:0,done:false});const {clock}=useThree();useFrame(()=>{const t=clock.getElapsedTime();p.current.intro=clamp(t/INTRO);p.current.done=t>=INTRO;if(p.current.done){const max=Math.max(document.documentElement.scrollHeight-window.innerHeight,1);p.current.scroll=clamp(window.scrollY/max)}});return <><color attach="background" args={['#73bced']}/><fog attach="fog" args={['#a9d5f2',22,90]}/><Sky distance={450000} sunPosition={[-20,28,12]} turbidity={5} rayleigh={1.8} mieCoefficient={.012} mieDirectionalG={.82}/><hemisphereLight color="#dff2ff" groundColor="#020202" intensity={.8}/><ambientLight intensity={.28}/><directionalLight position={[-12,20,10]} intensity={4.5} color="#fff" castShadow/><directionalLight position={[14,7,-10]} intensity={1.8} color="#8ebcff"/><Camera p={p}/><CloudsScene p={p}/><Runway/><Aircraft p={p}/><EffectComposer multisampling={0}><Bloom intensity={.3} luminanceThreshold={.82} luminanceSmoothing={.25} mipmapBlur/><Vignette darkness={.42} eskil={false}/></EffectComposer></> }
